...
@with_kw mutable struct Constants
    ...
    κ::Float64 = 1.0
    κf::Function = ce -> 4.1253e-2 + 500.7 * ce * (1e-6) - 4.7212e5 * ce^2 * 1e-12 +
                         1.5094e8 * ce^3 * (1e-18) - 1.6018e10 * ce^4 * 1e-24
    Uocp::Function = (Electrode, θ) -> if Electrode == "Neg"
        Uocp = @. -0.16 + 1.32 * exp(-3 * θ) + 10 * exp(-2000 * θ)
    else
        Uocp = @. 4.19829 + 0.0565661 * tanh(-14.5546 * θ + 8.60942) -
                  0.0275479 * (1 / (0.998432 - θ)^0.492465 - 1.90111) -
                  0.157123 * exp(-0.04738 * θ^8) + 0.810239 * exp(-40 * (θ - 0.133875))
    end

    ∂Uocp::Function = (Electrode, θ) -> if Electrode == "Neg"
        ∂Uocp = @. -20000 * exp(-2000 * θ) - 3.96 * exp(-3 * θ)
    else
        ∂Uocp = @. -32.4096 * exp(-40 * (-0.133875 + θ)) -
                   0.0135664 / ((0.998432 - θ)^1.49247) +
                   0.0595559 * exp(-0.04738 * θ^8) * θ^7 -
                   0.823297 * (sech(8.60942 - 14.5546 * θ))^2
    end
    Ce_M::Int64 = 4
    D1::Float64 = 1.0
    D2::Float64 = 1.0
...
@with_kw mutable struct TransferFun
    tfs = [[C_e, Phi_e, C_se, Phi_s, Phi_se, Flux, C_se, Phi_s, Flux,Phi_se] ["Na",
        "Na", "Pos", "Pos", "Pos", "Pos", "Neg", "Neg", "Neg","Neg"] [Number[
            0, 128e-6, 204e-6, 394e-6],
        Number[128e-6, 204e-6, 394e-6], Number[0, 1], Number[1], Number[0, 1],
        Number[0, 1], Number[0, 1], Number[1], Number[0, 1],Number[0, 1]]]
...
CellData = Cell(
    Constants(), Negative(), Positive(), Seperator(), RealisationAlgorthim(), TransferFun())
CellData.Const.Lnegsep, CellData.Const.Ltot = CellData.Neg.L + CellData.Sep.L,
CellData.Neg.L + CellData.Sep.L + CellData.Pos.L
