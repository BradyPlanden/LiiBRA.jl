...
@with_kw mutable struct Constants
    ...
    tpf::Function = ce -> -0.1287 * (ce / 1000)^3 + 0.4106 * (ce / 1000)^2 -
                          0.4717 * (ce / 1000) + 0.4492 # Transference Number Function - Requires ce in dm-3
    ...
    Ea_κ::Float64 = 0.0
    Ea_De::Float64 = 0
    CC_A::Float64 = 1.54e-4 #Electrode Plate Area 
    κ::Float64 = 0.9487
    κf::Function = ce -> 0.1297 * (ce / 1000)^3 - 2.51 * (ce / 1000)^1.5 +
                         3.329 * (ce / 1000) #Requires ce in dm-3
...
@with_kw mutable struct Negative
    L::Float64 = 7e-4    # Negative Electrode Length
    ...
    σ::Float64 = 1.08e7    # Solid Phase Conductivity
    ...
    α::Float64 = 0.3    # Alpha Factor
    k_norm::Float64 = 3.5e-8 # Reaction Rate
...
@with_kw mutable struct Positive
    L::Float64 = 4.2e-5    # Positive Electrode Length
...
@with_kw mutable struct Seperator
    L::Float64 = 2.5e-5  # Seperator Length
...
@with_kw mutable struct TransferFun
    ...
    # 4 - 2
    tfs::Vector{Function} = [
        C_e, Phi_e, C_se, Phi_s, Phi_se, Flux, C_se, Phi_s, Flux, Phi_se]
    Elec::Vector{String} = [
        "Na", "Na", "Pos", "Pos", "Pos", "Pos", "Neg", "Neg", "Neg", "Neg"]
    Locs::Vector{Vector{Float64}} = [
        Float64[0, 8.52E-05, 9.72E-05, 1.728E-04], Float64[8.52E-05, 9.72E-05, 1.728E-04],
        Float64[0, 1], Float64[1], Float64[0, 1], Float64[0, 1],
        Float64[0, 1], Float64[1], Float64[0, 1], Float64[0, 1]]
...
Cell = Params(
    Constants(), Negative(), Positive(), Seperator(), Realisation(), TransferFun())
