function C_e(Cell,s,z,tf,D,res0)
    """ 
...
function flambda(Cell,λ)
    ...
    sle1 = sqrt(λ*Cell.Neg.ϵ_e/Cell.Const.D1)
    sle2 = sqrt(λ*Cell.Sep.ϵ_e/Cell.Const.D2)
    sle3 = sqrt(λ*Cell.Pos.ϵ_e/Cell.Const.D3)
    k3 = @. (cos(sle1*Cell.Neg.L)*cos(sle2*Cell.Neg.L) + Cell.Const.D1*sle1*sin(sle1*Cell.Neg.L)*sin(sle2*Cell.Neg.L)/(Cell.Const.D2*sle2))
    k4 = @. (cos(sle1*Cell.Neg.L)*sin(sle2*Cell.Neg.L) - Cell.Const.D1*sle1*cos(sle2*Cell.Neg.L)*sin(sle1*Cell.Neg.L)/(Cell.Const.D2*sle2))
    k5 = @. k3*(cos(sle2*Lnegsep)*cos(sle3*Lnegsep) + Cell.Const.D2*sle2*sin(sle2*Lnegsep)*sin(sle3*Lnegsep)/(Cell.Const.D3*sle3))+k4*(sin(sle2*Lnegsep)*cos(sle3*Lnegsep) - Cell.Const.D2*sle2*cos(sle2*Lnegsep)*sin(sle3*Lnegsep)/(Cell.Const.D3*sle3))
    k6 = @. k3*(cos(sle2*Lnegsep)*sin(sle3*Lnegsep) - Cell.Const.D2*sle2*sin(sle2*Lnegsep)*cos(sle3*Lnegsep)/(Cell.Const.D3*sle3))+k4*(sin(sle2*Lnegsep)*sin(sle3*Lnegsep) + Cell.Const.D2*sle2*cos(sle2*Lnegsep)*cos(sle3*Lnegsep)/(Cell.Const.D3*sle3))
    Psiprime = @. -k5*sle3*sin(sle3*Cell.Const.Ltot) + k6*sle3*cos(sle3*Cell.Const.Ltot)
